...
impl PreviewUi {
    ...
    fn show_image_info(&self) {
        ...
        match source {
            ...
            PreviewSource::LiveStacking => {
                let ls_info = self.engine.live_stacking.info.read().unwrap();
    ...
    fn zoom_by_mouse(&self, evt: &gdk::EventScroll) {
        ...
        let Ok(mut options) = self.engine.options.try_write() else {
            return;
        };
    ...
    fn handler_action_save_image_linear(&self) {
        exec_and_show_error(Some(&self.window), || {
            ...
            match preview_source {
                ...
                PreviewSource::LiveStacking => {
                    let stacker = &self.engine.live_stacking.stacker;
        ...
        let model = match tree.model() {
            ...
            None => {
                init_list_store_model_for_treeview(tree, &[
                    ...
                    /* 7 */  ("Backgr.",  String::static_type(), "text"),
        ...
        fc.connect_response(clone!(@weak self as self_ => move |file_chooser, response| {
            if response == gtk::ResponseType::Accept {
                exec_and_show_error(Some(&self_.window), || {
                    ...
                    self_.engine.open_image_from_file(&file_name)?;
                    Ok(())
                });
            }
            file_chooser.close();
